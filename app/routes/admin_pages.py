    ...
    if current_user.admin_acc == True:
        ...
        if request.method == 'POST':
            ...
            for image in parse_images(id):
                ...
                image_string += '<img src="'+img_link+'" alt="img" onclick="delete_img('+str(id)+')" style="cursor: pointer;" id="image_delete_'+str(id)+'">'
                i+=1
            print(image_string)
            try:
                post = Blog_Post.query.get_or_404(id)
                ...
                image_sql = ""
            return render_template("edit.html", title="Edit", post=post, images=image_string, image_sql=image_sql)
    else:
        return render_template("error.html", title="Error", error="You're not allowed to use that page!")
